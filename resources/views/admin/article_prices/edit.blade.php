...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3" style="background-color:#6c757d;">
            <h5 class="m-0 font-weight-bold text-white">Modifier le Prix Fournisseur #{{ $articlePrice->id }}</h5>
        </div>
        <form action="{{ route('articlePriceUpdate', $articlePrice->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="reference" id="reference" class="form-control @error('reference') is-invalid @enderror" value="{{ old('reference', $articlePrice->reference) }}">
                            @error('reference')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="designation" id="designation" class="form-control @error('designation') is-invalid @enderror" value="{{ old('designation', $articlePrice->designation) }}" required>
                            @error('designation')
                                <small class="invalid-feedback">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="unit_price_display" class="form-control" value="{{ old('unit_price', number_format($articlePrice->unit_price, 0, '.', ' ')) }}" placeholder="Ex : 1 000" required>
                            <input type="hidden" name="unit_price" id="unit_price" value="{{ old('unit_price', $articlePrice->unit_price) }}">
                            @error('unit_price')
                                <small class="invalid-feedback">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="type" id="type" class="form-control @error('type') is-invalid @enderror">
                                ...
                                <option value="Originale" {{ old('type', $articlePrice->type) == 'Originale' ? 'selected' : '' }}>Originale</option>
                                <option value="Aftermarket" {{ old('type', $articlePrice->type) == 'Aftermarket' ? 'selected' : '' }}>Aftermarket</option>
                            </select>
                            @error('type')
                                <small class="invalid-feedback">{{ $message }}</small>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="currency" id="currency" class="form-control @error('currency') is-invalid @enderror">
                                @foreach(['FCFA', 'Dollar', 'Euro', 'Livre Sterling'] as $currency)
                                    <option value="{{ $currency }}" {{ old('currency', $articlePrice->currency) == $currency ? 'selected' : '' }}>{{ $currency }}</option>
                                @endforeach
                            </select>
                            @error('currency')
                                <small class="invalid-feedback">{{ $message }}</small>
                    ...
                    <div class="col-6">
                        <div class="mb-3">
                            ...
                            <select name="supplier_id" id="supplier_id" class="form-control @error('supplier_id') is-invalid @enderror" onchange="toggleOtherSupplier(this)">
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" {{ old('supplier_id', $articlePrice->supplier_id) == $supplier->id ? 'selected' : '' }}>{{ $supplier->company_name }}</option>
                                @endforeach
                                <option value="other" {{ old('supplier_name', $articlePrice->supplier_name) ? 'selected' : '' }}>Autres</option>
                            </select>
                            @error('supplier_id')
                                <small class="invalid-feedback">{{ $message }}</small>
                            @enderror
                        ...
                        <div class="mb-3" id="otherSupplierDiv" style="{{ old('supplier_name', $articlePrice->supplier_name) ? 'display:block;' : 'display:none;' }}">
                            <input type="text" name="supplier_name" id="supplier_name" class="form-control @error('supplier_name') is-invalid @enderror" placeholder="Saisir le fournisseur..." value="{{ old('supplier_name', $articlePrice->supplier_name) }}">
                            @error('supplier_name')
                                <small class="invalid-feedback">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="date" id="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $articlePrice->date ? $articlePrice->date->format('Y-m-d') : '') }}">
                            @error('date')
                                <small class="invalid-feedback">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="incoterm" id="incoterm" class="form-control @error('incoterm') is-invalid @enderror">
                                ...
                                @foreach(['EXW', 'FCA', 'FAS', 'FOB', 'CFR', 'CIF', 'CPT', 'CIP', 'DAP', 'DPU', 'DDP'] as $term)
                                    <option value="{{ $term }}" {{ old('incoterm', $articlePrice->incoterm) == $term ? 'selected' : '' }}>{{ $term }}</option>
                                @endforeach
                            </select>
                            @error('incoterm')
                                <small class="invalid-feedback">{{ $message }}</small>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="country" id="country" class="form-control @error('country') is-invalid @enderror">
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country }}" {{ old('country', $articlePrice->country) == $country ? 'selected' : '' }}>{{ $country }}</option>
                                @endforeach
                            </select>
                            @error('country')
                                <small class="invalid-feedback">{{ $message }}</small>
                        ...
                        <div class="row mt-4">
                            <div class="col-6">
                                <button type="submit" class="btn w-100 text-white" style="background-color: rgb(249, 138, 27); border: none;">Mettre à jour</button>
                            </div>
                            <div class="col-6">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    displayInput.addEventListener('input', function () {
        ...
        // Vérifier si c'est un nombre avant de traiter
